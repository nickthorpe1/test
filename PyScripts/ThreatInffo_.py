...
EXCEL_FILE = 'threat_actor_info_onelasttry.xlsx'
...
# Function to get threat actor information, including attack_patterns
def get_threat_actor_info(token, actor_name):
    ...
    try:
        ...
        # Extract attack patterns if available
        if 'attack_patterns' in data:
            attack_patterns = data['attack-pattern']
            print(f"\nAttack Patterns for {actor_name}:")
            for pattern in attack_patterns:
                print(f"- {pattern.get('name')} (ID: {pattern.get('external_id')})")
        else:
            attack_patterns = []
